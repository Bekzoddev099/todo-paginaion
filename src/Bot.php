...
class Bot
{
    ...
    {
        $status = 'add';
        $query  = "INSERT INTO list (chat_id, status, created_at)
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id FROM list where chat_id = :chat_id LIMIT 1");
        ...
        $status = null;
        $stmt   = $this->pdo->prepare("UPDATE list SET status=:status WHERE chat_id = :chatId");
    ...
    {
        $query = "SELECT * FROM todos WHERE user_id = (SELECT id FROM list WHERE chat_id = :chatId)";
        $stmt  = $this->pdo->prepare($query);
        $stmt->bindParam(':chatId', $chatId);
        $stmt->execute();
